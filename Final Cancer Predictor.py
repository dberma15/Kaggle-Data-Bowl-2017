...
'''
parentdirectory="C:\\Users\\daniel\\Documents\\Machine Learning And Statistics Projects\\LungCancer\\stage2\\stage2"
...
model=load_model("C:\\Users\\daniel\\Documents\\Machine Learning And Statistics Projects\\LungCancer\\stage1\\stage1\\DataBowl2017 modelLSTM_frog_classifier_20170331 081917.h5")
...
if preprocessTestingData:
	...
	if labeledTestingData:
		...
		np.save("y_validation2.npz",y_testing)
	if not labeledTestingData:
		...
		with open("validationOrder2.txt","wb") as fp:
			pickle.dump(directoryOrder,fp)
			
	np.save("X_validation2.npz",X_testing)
else:
	X_testing=np.load("X_validation2.npz.npy")
	if labeledTestingData:
		y_testing=np.load("y_validation2.npz.npy")
	else:
		with open("validationOrder2.txt",'rb') as fp:
			directoryOrder=pickle.load(fp)

X_testing=X_testing[:,0:541,]
predictions=model.predict(X_testing)

print(predictions)
results=pandas.DataFrame({'id':pandas.Series(directoryOrder),'cancer':pandas.Series(predictions.reshape(506))})
